...
export const RecycleBin = () => {
  ...
  const fetchAllUsers = async () => {
    try {
      const { data } = await axios.get("/api/v1/user/all-deleted-users", {
        headers,
      });
      if (data?.success) {
        // data.users.map(
  ...
  const handleDelete = async (id, name) => {
    try {
      if (
        window.confirm("Do you really want to restore this student record?")
      ) {
        let token = localStorage.getItem("token");
        const headers = { Authorization: "Bearer " + token };
        console.log(headers);
            ...
            >
              <div className="mt-3">
                <h4 className="font-weight-bold mb-3">
                  Student Registration - Recycle Bin
                </h4>
